      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {tournaments.map((tournament, index) => (
            <Reveal key={tournament.id} delay={index * 0.15}>
              <div className="group relative bg-gray-900 overflow-hidden border border-white/5 hover:border-swift-coral transition-colors duration-500 h-full flex flex-col">
                {/* Status Badge */}
                <div className={`absolute top-4 right-4 z-20 px-3 py-1 text-xs font-bold uppercase tracking-wider ${
                  tournament.status === 'open' ? 'bg-swift-coral text-white' : 'bg-gray-800 text-gray-400'
                }`}>
